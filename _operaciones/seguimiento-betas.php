...
if (isset($_POST["tipo"]) && !empty($_POST["tipo"])) {
	switch ($_POST["tipo"]) {
		case 'obtener_data':
			...
			echo json_encode(["message" => obtenerData($_POST)]);
			break;
		case 'get_item':
			http_response_code(200);
...
function obtenerData($data) {
	return ['content' => obtenerHtml(obtenerQuery($data))];
}
function obtenerQuery($data) {
	global $db;
	$stmt=$db->prepare("CALL MedicoReporteBetas(?, ?, ?);");
	$stmt->execute([$data["tipo_fecha"], $data["ini"], $data["fin"]]);
	return $stmt->fetchAll(PDO::FETCH_ASSOC);
}
function obtenerHtml($data) {
	$html='';
	foreach ($data as $key => $value) {
		...
		$html.='<td>';
		/* $html.='<a href="man-tipo-cambio-edit.php?id='.$value["protocolo"].'">';
		$html.='<img src="_libraries/open-iconic/svg/pencil.svg" height="18" width="18" alt="icon name">';
		$html.='</a>'; */
		$html.='<a href="javascript:void(0)">';
		$html.='<img src="_libraries/open-iconic/svg/pencil.svg" data-id="'.$value["protocolo"].'" class="form-confirm-delete" height="18" width="18" alt="icon name">';
		$html.='</a>';
		$html.='</td>';
		$html.='</tr>';
	}
