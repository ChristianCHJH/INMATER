...
<div data-role="page" class="ui-responsive-panel" id="r_tanque" data-dialog="true">
    ...
    <?php
        ...
        if (isset($_POST['conf'])) {
            if ($_POST['conf'] <> "") {
                man_extras_eliminar($_POST['conf'], mb_strtolower($login));
            }
        }
        if (isset($_POST['nom'])) {
            if ($_POST['nom'] <> "") {
                man_extras_insertar(mb_strtolower($_POST['nom']), mb_strtolower($login));
            }
        }
    	$rPare = $db->prepare("select * from man_extras where estado = 1");
    	$rPare->execute();
	?>
    <style>
        .ui-dialog-contain {
    ...
    <div data-role="header">
        ...
        <h3>MANTENIMIENTO DE EXTRAS</h3>
    </div>
    <div class="ui-content" role="main">
        <form action="m_extras.php" method="post" data-ajax="false" name="form1">
        ...
        <form action="m_extras.php" method="post" data-ajax="false" name="form2">
