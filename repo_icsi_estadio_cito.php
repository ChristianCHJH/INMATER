...
<?php
    if ($_SESSION['role'] == "9") {
        ...
        $rPaci = $db->prepare("
            ...
            , lab_aspira.fec
            , count( case when (lab_aspira_dias.d0est = 'CT' or lab_aspira_dias.d1est = 'CT') then true end ) atresico
            from hc_reprod
            ...
            inner join lab_aspira_dias on lab_aspira_dias.pro = lab_aspira.pro and lab_aspira_dias.estado is true and (lab_aspira_dias.d0est = 'CT' or lab_aspira_dias.d1est = 'CT')$between
            ...
            , lab_aspira.tip, hc_reprod.p_cic, hc_reprod.p_fiv, hc_reprod.p_icsi, hc_reprod.p_od, hc_reprod.p_don, hc_reprod.p_cri, hc_reprod.p_iiu
            , lab_aspira.fec
            having count( case when (lab_aspira_dias.d0est = 'CT' or lab_aspira_dias.d1est = 'CT') then true end ) >= 1
        ...
        while ($paci = $rPaci->fetch(PDO::FETCH_ASSOC)) {
            ...
            print("
                <td>".$paci["med"]."</td>
                <td>".$paci["fec"]."</td>
                <td><a href='le_aspi1.php?id=".$paci['pro']."' target='_blank'>".$paci["atresico"]."</a></td></tr>");
        }
        print("
            </tbody>
        </table>");
        ...
        <div class="ui-content" role="main" id="imprime">
            ...
            <div class="scroll_h">
                <table style="border: 1px solid;" cellpadding="5">
                    <tbody>
                    <?php
                        print '<tr><td>Embriones Citolizados</td><td>'.$atresico.'</td></tr>';
                    ?>
                        <tr>
                            <td colspan="2" bgcolor="#ffe4c4"><?php print("Total:".($atresico) ); ?></td>
                        </tr>
                    </tbody>
                </table><br/><br/>
            </div>
