...
if (isset($_POST['n_ovo']) and $_POST['guardar'] == "GUARDAR DATOS") {
    ...
    if ($c > 0) {
        ...
        for ($i = 1; $i <= $c; $i++) {
            if ($_POST['anu' . $i] == 0 or $_POST['anu' . $i] == 7) {
                ...
                lab_updateAspi_d6($_POST['pro'], $i, $anu, $_POST['cel' . $i], $_POST['mci' . $i], $_POST['tro' . $i], $_POST['fra' . $i], $_POST['vac' . $i], $_POST['colap' . $i], $_POST['d_bio' . $i], $_POST['kid' . $i], $_POST['kid' . $i . '_tipo'], $_POST['kid' . $i . '_decimal'], $_POST['f_cic' . $i], $_POST['obs' . $i], $_POST['T' . $i], $_POST['C' . $i], $_POST['G' . $i], $_POST['P' . $i], $_POST['col' . $i], $don, $_FILES['i' . $i]);
            }
        }
    }
    ...
    if ($_POST['Tra'] == 1) {
        lab_updateAspi_sta_T($_POST['Tra_id'], $_POST['pro'], 6, $_POST['T_t_cat'], $_POST['T_s_gui'], $_POST['T_s_cat'], $_POST['T_endo'], $_POST['T_inte'], $_POST['T_eco'], $_POST['T_med'], $_POST['T_emb'], $_POST['T_obs'], $login);
    }

    if ($_POST['pro'] and isset($_FILES) and ((isset($_FILES['vid_embry']) and !empty($_FILES['vid_embry'])) or (isset($_FILES['pdf_embry']) and !empty($_FILES['pdf_embry'])))) {
        lab_insertEmbry($_POST['pro'], $_FILES['vid_embry'], $_FILES['pdf_embry'], $_POST['rep'], $login);
...
if ($_GET['id'] <> "") {
    ...
    $rPaci = $db->prepare("SELECT
        lab_aspira.*,
        hc_reprod.id,hc_reprod.eda,hc_reprod.p_cic,hc_reprod.p_fiv,hc_reprod.p_icsi,hc_reprod.p_od,hc_reprod.p_don,hc_reprod.p_cri,hc_reprod.p_extras,hc_reprod.p_notas,hc_reprod.pago_extras,hc_reprod.pago_notas,hc_reprod.p_dni,hc_reprod.med
        FROM lab_aspira
    ...
    if (isset($paci['p_dni']) && $paci['p_dni'] == "") {$pareja = "SOLTERA";} else {$pareja = (isset($hombre['p_ape']) ? $hombre['p_ape'] :"") . " " . (isset($hombre['p_nom']) ? $hombre['p_nom'] :"");}

    $campos = "ovo, anu, d0est, d6cel, d6mci, d6tro, d6fra, d6vac, d6col, d6d_bio, d6kid, d6kid_tipo, d6kid_decimal, d6f_cic, obs, t, c, g, p, col, des, don";
    ...
    <form action="" method="post" enctype="multipart/form-data" data-ajax="false" id="form1">
        <div data-role="page" class="ui-responsive-panel" id="le_aspi6">
            ...
            <div data-role="header" data-position="fixed">
                ...
                <h2>
                    <?php
                    if ($paci['tip'] == 'T') {echo "(" . $paci['pro'] . ")";} else {echo "(" . $paci['tip'] . "-" . $paci['pro'] . "-" . $paci['vec'] . ")";}
                    echo " Dia 6"; if ($paci['fec6'] <> '1899-12-30') {echo ': <small>'.date("d-m-Y", strtotime($paci['fec6'])).'</small>';}
                ?>
                </h2>
                <a href="salir.php"
            ...
            <div class="ui-content" role="main">
                            ...
                            <td width="7%">Embriologo
                                <select name="emb6" required id="emb6" data-mini="true">
                                    ...
                                    <option value=<?php echo $embrio['id'];
                                        if ($paci['emb6'] == $embrio['id']) echo " selected"; ?>>
                        ...
                        <tr>
                            ...
                            <td width="5%">
                                ...
                                <select name="emb6c" id="emb6c" data-mini="true">
                                    ...
                                    <?php foreach ($rows as $embrio) { ?>
                                    <option value=<?php echo $embrio['id'];
                                        if ($paci['emb6c'] == $embrio['id']) echo " selected"; ?>>
                                        <?php echo $embrio['nom']; ?></option>
                                    <?php } ?>
                                </select>
                            </td>
                ...
                <div data-role="collapsible" data-mini="true" class="tran">
                    <h1>TRANSFERENCIA</h1>
                    <input type="hidden" name="Tra_id" value="<?php if(isset($tra['pro']))echo $tra['pro']; ?>">
                    <table width="100%" align="center" style="margin: 0 auto; font-size: small;">
                        <tr>
                            <td width="19%">
                                Tipo de cateter
                            ...
                            <td width="10%">Sangre en cateter
                                <select name="T_s_cat" id="T_s_cat" data-mini="true">
                                    <option value=0 <?php if (isset($tra['s_cat']) && $tra['s_cat'] == 0) {echo "selected";} ?>>0</option>
                                    <option value=1 <?php if (isset($tra['s_cat']) && $tra['s_cat'] == 1) {echo "selected";} ?>>1</option>
                                    <option value=2 <?php if (isset($tra['s_cat']) && $tra['s_cat'] == 2) {echo "selected";} ?>>2</option>
                                    <option value=3 <?php if (isset($tra['s_cat']) && $tra['s_cat'] == 3) {echo "selected";} ?>>3</option>
                                </select>
                            </td>
                            <td width="5%">Endometrio
                                <select name="T_endo" id="T_endo" data-mini="true">
                                    <option value=0 <?php if (isset($tra['endo']) && $tra['endo'] == 0) {echo "selected";} ?>>Seleccionar
                                    </option>
                                    <option value=5 <?php if (isset($tra['endo']) && $tra['endo'] == 5) {echo "selected";} ?>>Menor/igual a 5
                                        mm
                                    </option>

                                    <option value=6 <?php if (isset($tra['endo']) && $tra['endo'] == 6) {echo "selected";} ?>>6mm</option>
                                    <option value=7 <?php if (isset($tra['endo']) && $tra['endo'] == 7) {echo "selected";} ?>>7mm</option>
                                    <option value=8 <?php if (isset($tra['endo']) && $tra['endo'] == 8) {echo "selected";} ?>>8mm</option>
                                    <option value=9 <?php if (isset($tra['endo']) && $tra['endo'] == 9) {echo "selected";} ?>>9mm</option>
                                    <option value=10 <?php if (isset($tra['endo']) && $tra['endo'] == 10) {echo "selected";} ?>>10mm</option>
                                    <option value=11 <?php if (isset($tra['endo']) && $tra['endo'] == 11) {echo "selected";} ?>>11mm</option>
                                    <option value=12 <?php if (isset($tra['endo']) && $tra['endo'] == 12) {echo "selected";} ?>>12mm</option>
                                    <option value=13 <?php if (isset($tra['endo']) && $tra['endo'] == 13) {echo "selected";} ?>>13mm</option>
                                    <option value=14 <?php if (isset($tra['endo']) && $tra['endo'] == 14) {echo "selected";} ?>>Mayo/igual a
                                        14mm
                                    </option>
                                </select>
                            </td>
                            <td width="5%">Intentos
                                <select name="T_inte" id="T_inte" data-mini="true">
                                    <option value=0 <?php if (isset($tra['inte']) && $tra['inte'] == 0) {echo "selected";} ?>>Seleccionar
                                    </option>
                                    <option value=1 <?php if (isset($tra['inte']) && $tra['inte'] == 1) {echo "selected";} ?>>1</option>
                                    <option value=2 <?php if (isset($tra['inte']) && $tra['inte'] == 2) {echo "selected";} ?>>2</option>
                                    <option value=3 <?php if (isset($tra['inte']) && $tra['inte'] == 3) {echo "selected";} ?>>3</option>
                                </select>
                            </td>
                                ...
                                <select name="T_eco" id="T_eco" data-mini="true">
                                    ...
                                    <?php
                                        ...
                                        while ($data = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                            ...
                                            if (isset($tra['eco']) && isset($data['codigo']) && $tra['eco'] == $data['codigo']) $selected = "selected";
                            ...
                            <td width="6%">Embriologo
                                <select name="T_emb" id="T_emb" data-mini="true">
                                    ...
                                    <?php foreach ($rows as $embrio) { ?>
                                    <option value=<?php echo $embrio['id'];
                                            if(isset($tra['emb']))if ($tra['emb'] == $embrio['id']) echo " selected"; ?>>
                                        <?php echo $embrio['nom']; ?></option>
                                    <?php } ?>
                                </select>
                            </td>
                            <td width="39%" align="center" valign="top">Observaciones
                                <textarea name="T_obs" id="T_obs"><?php if(isset($tra['obs']))echo $tra['obs']; ?></textarea>
                            </td>
                        </tr>
                    </table>
                </div>
                    ...
                    <span class="embry" style="font-size: small">
                        <?php
                            ...
                            if (empty($link_video) && file_exists("emb_pic/embryoscope_" . $paci['pro'] . ".mp4")) {
                                $link_video = "<a href='embryoscope_" . $paci['pro'] . ".mp4' target='new'>(VER)</a>";
                            }
                    ...
                    <table data-role="table" style="margin: 0 auto;font-size:small;" class="ui-responsive table-stroke">
                        ...
                        <tr <?php if ($aspi['anu'] > 0) {
                                ...
                                <select name="cel<?php echo $c; ?>" class="val_defect" title="<?php echo $c; ?>"
                                    ...
                                    <?php
                                                $consulta = $db->prepare("SELECT
                                                    c.codigo, c.nombre, COALESCE(p6.id, 0) pred
                                                    FROM lab_celulas c
                                                    LEFT JOIN lab_celulas p6 ON p6.id = c.id AND p6.dia6predeterminado = 1 AND p6.estado = 1
                                                    WHERE c.estado=1 AND c.dia6=1
                                                    ORDER BY pred DESC, c.id ASC");
                                                $consulta->execute();
                                                $data = $consulta->fetchAll();
                                                foreach ($data as $item) { ?>
                                    <option value="<?php print($item['codigo']); ?>"
                                        <?php if ($aspi['d6cel'] == $item['codigo']) print(" selected"); ?>>
                            ...
                            <td>
                                <select class="form-control" name="colap<?php echo $c; ?>" data-mini="true" id="colap">
                                    ...
                                    <option value=<?php echo $sino['codigo'];
                                                    if ($aspi['d6col'] == $sino['codigo']) {echo " selected";} ?>>
                            ...
                            <td>
                                <select class="form-control" name="d_bio<?php echo $c; ?>" data-mini="true"
                                    ...
                                    <?php
                                                ...
                                                foreach ($rows as $sino) {?>
                                    <option value=<?php echo $sino['codigo'];
                                                    if ($aspi['d6d_bio'] == $sino['codigo']) echo " selected"; ?>>
                                        <?php echo $sino['nombre']; ?></option>
                                    <?php } ?>
                                </select>
                            </td>
                            ...
                            <td>
                                <select name="kid<?php echo $c; ?>_tipo" data-mini="true">
                                    ...
                                    <option value=<?php echo $item['id'];
                                                if ($aspi['d6kid_tipo'] == $item['id']) echo " selected"; ?>>
                            ...
                            <td><input type="number" name="kid<?php echo $c; ?>_decimal"
                                    id="kid<?php echo $c; ?>_decimal" value="<?php echo $aspi['d6kid_decimal']; ?>"
                            ...
                            <td class="enlinea">
                                ...
                                <select name="f_cic<?php echo $c; ?>" class="f_cic" title="<?php echo $aspi['ovo']; ?>"
                                    ...
                                    <option value="N"
                                        <?php if ($aspi['d6f_cic'] == "N" or $aspi['d6f_cic'] == "") {echo "selected";} ?>>
                                        NV</option>
                            ...
                            <td>
                                ...
                                <?php
                                        if (file_exists("emb_pic/p" . $paci['pro'] . "d6_" . $aspi['ovo'] . ".jpg")) {
                                ...
                                <div data-role="popup" id="f<?php echo $c; ?>" class="ui-content"> Subir/Cambiar
                                    ...
                                    <input name="i<?php echo $c; ?>" type="file" accept="image/jpeg" data-mini="true"
                                        class="fotox" /> Observaciones
                                    <textarea name="obs<?php echo $c; ?>" id="obs"
                ...
                <?php
                    if (strpos($paci['pago_extras'], "NGS") !== false) {
                        print('<b><small>, <i>Informe NGS:</i></small></b> <a href="info_ngs_biopsia.php?repro='.$paci['rep'].'&path=le_aspi6&pro='.$paci['pro'].'" rel="external"><i class="far fa-file-alt"></i></a>');
                        print('<b><small>, <i>Informe IGenomix:</i></small></b> <a href="igeno_informe.php?path=le_aspi6&pro='.$paci['pro'].'" rel="external"><i class="far fa-file-alt"></i></a>');
