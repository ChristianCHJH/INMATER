<?php
	...
	exit();*/
	$idmedico = $tipaspi = $anio = $mes = "";
	$asp1 = $asp2 = $asp3 = $asp4 = $asp5 = 0;
	// $arrayName = ["80", "90"];
	// print_r(json_encode($arrayName));
	if (isset($_POST["idmedico"]) && isset($_POST["anio"]) && !empty($_POST["anio"]) && isset($_POST["tipaspi"]) && !empty($_POST["tipaspi"]) && isset($_POST["mes"]) && !empty($_POST["mes"])) {
    ...
    $rPaci = $db->prepare("
        ...
        where hc_reprod.estado = true and cancela=0 AND hc_reprod.med='$idmedico' AND CAST(hc_reprod.f_asp as date) BETWEEN '$fechaini' AND '$fechafin'
        group by coalesce(lab_aspira.tip, 'D'), hc_reprod.des_don, hc_reprod.des_dia, don_todo, p_cri, p_fiv, p_icsi, pago_extras
        , hc_reprod.p_dni_het-- , coalesce(lab_andro_cap.id, 0)
	...
	switch ($tipaspi) {
		case '1': print($asp1); break;
		case '2': print($asp2); break;
		case '3': print($asp3); break;
		case '4': print($asp4); break;
		case '5': print($asp5); break;
	}
?>