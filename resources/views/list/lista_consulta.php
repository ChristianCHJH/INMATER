...
<body>
    ...
    if (isset($_POST['btn_consulta']) and $_POST['btn_consulta'] == "AGENDAR CONSULTA" and isset($_POST['dni']) and isset($_POST['fec']) and isset($_POST['fec_h'])) {
        ...
        $stmt = $db->prepare(
            ...
            (dni, fec, med, fec_h, fec_m, mot, cupon, tipoconsulta_ginecologia_id, man_motivoconsulta_id, idusercreate) VALUES
            (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
        );

        $stmt->execute([$_POST['dni'], $_POST['fec'], $_POST['med_agenda'], $_POST['fec_h'], $_POST['fec_m'], $_POST['mot'], (isset($_POST['cupon']) ? $_POST['cupon'] : 11), $_POST['tipoconsulta_id'], $_POST['man_motivoconsulta_id'], $login]);
        $idp = $db->lastInsertId();
