        ...
        <div class="ui-content" role="main">
            ...
            { ?>
                ...
                $rAnal = $db->prepare("SELECT
                    ...
                    WHERE hc_analisis.a_dni=hc_paciente.dni AND CAST(a_fec AS DATE) = CAST(CURRENT_TIMESTAMP AS DATE) AND  hc_analisis.a_med ilike ?");
            ...
            (
                ...
                where hc_paciente.dni=hc_reprod.dni and split_part(hc_reprod.f_asp, 'T', 1) = ? and coalesce(hc_reprod.cancela, 0) <> 1
                ...
                left join lab_aspira on lab_aspira.rep = hc_reprod.id and lab_aspira.estado is true
                where hc_paciente.dni=hc_reprod.dni and hc_reprod.f_tra=? and coalesce(hc_reprod.cancela, 0) <> 1
                        ...
                        <table style="font-size:14px;width:100%;" class="tabla-agenda-inmater table-stripe ui-responsive">
                            ...
                            <tbody>
                                ...
                                <td>
                                    ...
                                    <a href='<?php echo $url; ?>' target="_blank" rel="external"><?php echo $rep['nombres_completos']; ?></a>
                                    <?php
                                ...
                                <td>
                                    ...
                                    $var = ['0'=>"", "1" => "Procedimiento sin anestesia", "2" => "Procedimiento bajo anestesia"];
            ...
            if ($user['role'] == 3 || $user['role'] == 10 || $user['role'] == 19 || $user['role'] == 20) {
                if(isset($_POST['anu_x']))if ($_POST['anu_x'] <> "" and $_POST['anu_y'] <> "") {
                    $stmt = $db->prepare("UPDATE recibos SET anu=1,iduserupdate=?,updatex=? WHERE id=? AND tip=?");
                    $hora_actual = date("Y-m-d H:i:s");
                    $stmt->execute(array($login,$hora_actual,$_POST['anu_x'], $_POST['anu_y']));
                    ...
                    $log_Recibos = $db->prepare(
                        ...
                        WHERE id=? AND tip=?");
                    $log_Recibos->execute(array($_POST['anu_x'], $_POST['anu_y']));
                }
                ...
                if (isset($_POST['VER']) && $_POST['VER'] == "VER") {
                    ...
                    $rRec = $db->prepare("SELECT *
                        FROM recibos
                        WHERE 1=1 and sede_pago_id IN ($in) $t_pag$t_ser$med$fecha$apellidos_nombres
                        ORDER BY fec DESC, id DESC");
                    $rRec->execute($variables);
                } else {
                    $rRec = $db->prepare("SELECT *
                    from recibos
                    where sede_pago_id IN ($in)
                    order by fec desc, id desc
                    limit 50");
                    $rRec->execute($variables);
                }
                ...
                <div class="enlinea">
                    ...
                    <small>Apellidos o Nombres:</small><input type="text" data-mini="true" name="apellidos_nombres">
                            ...
                            <tr <?php if ($rec['anu'] == 1) echo 'bgcolor="#F9CCCD"'; ?>>
                                ...
                                <td><a href="#" data="<?php echo $rec['id']."_".$rec['tip']; ?>" class="mas">
                                    ...
                                    <div id="<?php echo $rec['id']."_".$rec['tip']; ?>" class="mas2">
                                        ...
                                        <?php if ($rec['man_ini'] <> '1970-01-02') echo 'Inicio:'.date("d-m-Y", strtotime($rec['man_ini'])).' Fin:'.date("d-m-Y", strtotime($rec['man_fin'])); ?>
                                    </div>
                                </td>
                                <td><?php
                                        if ($rec['t_ser'] == 1 or $rec['t_ser'] == 2 or $rec['t_ser'] == 3) {
                                ...
                                <td style="font-size:10px">
                                    ...
                                    <?php if ($rec['t_ser'] == 1) { ?><a href='<?php echo "pago_veri.php?id=".$rec['id']."&t=".$rec['tip']; ?>' rel="external" class="noVer">Verificar</a><?php } ?><br>
                                    <?php if ($rec['anu'] == 1) echo 'Anulado'; else { ?><a href="javascript:anular(<?php echo $rec['id'].','.$rec['tip']; ?>);" class="noVer">Anular</a><?php } ?>
                                </td>
                            </tr>
                            <?php } ?>
