...
<?php
    if ($_SESSION['role'] == "9") {
        ...
        $rPaci = $db->prepare("
            ...
            , count( case when lab_aspira_t.beta = 2 then true end ) betaneg
            from hc_reprod
            inner join lab_aspira on lab_aspira.rep = hc_reprod.id and lab_aspira.estado is true and lab_aspira.estado is true and lab_aspira.f_fin is not null and lab_aspira.tip <> 'T'
            ...
            , lab_aspira.tip, hc_reprod.p_cic, hc_reprod.p_fiv, hc_reprod.p_icsi, hc_reprod.p_od, hc_reprod.p_don, hc_reprod.p_cri, hc_reprod.p_iiu
            , lab_aspira.fec
            having count( case when lab_aspira_t.beta = 2 then true end ) > 0
        ...
        while ($paci = $rPaci->fetch(PDO::FETCH_ASSOC)) {
            ...
            print("
                <td>".$paci["med"]."</td>
                <td>".$paci["fec"]."</td>
                <td><a href='le_aspi1.php?id=".$paci['pro']."' target='_blank'>".$paci["betaneg"]."</a></td></tr>");
        }
        print("
            </tbody>
        </table>");
        ...
        <div class="ui-content" role="main" id="imprime">
            ...
            <div class="scroll_h">
                <table style="border: 1px solid;" cellpadding="5">
                    <tbody>
                    <?php
                        print '<tr><td>Betas FIV Negativas</td><td>'.$betaneg.'</td></tr>';
                    ?>
                        <tr>
                            <td colspan="2" bgcolor="#ffe4c4"><?php print("Total:".($betaneg) ); ?></td>
                        </tr>
                    </tbody>
                </table><br/><br/>
            </div>
