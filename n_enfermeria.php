...
<body>
	...
	<div class="container">
		...
		<div data-role="collapsible" id="Perfi">
			<?php print("<h4 class='color-primary'>Documentos Enfermería: <small>".mb_strtoupper($paci['ape'])." ".mb_strtoupper($paci['nom'])."</small></h4>");?>
			<div class="card mb-3">
				<h5 class="card-header">Agregar documentos Enfermería</h5>
				...
				<?php
					...
					if ($user['role'] == 1 || $user['role'] == 12 || $user['role'] == 13 || $user['role'] == 15) {
						print('<a href="n_enfermeria_add.php?dni='. $dni . '" rel="external" class="btn btn-danger">Agregar</a><br><br>');
					}
					$rLegal = $db->prepare("
						select
						e.id, e.finforme, e.documento, e.obs, m.nombre medico, mr.nombre nombre_reproasistida, mp.nombre nombre_procesala
						from hc_enfermeria e
						left join man_medico m on m.codigo = e.idmedico
						left join man_enfermeria_repro mr on mr.codigo = e.idreproasistida
						left join man_enfermeria_proce mp on mp.codigo = e.idprocesala
						where e.estado = 1 and e.numerodocumento = ?
						order by e.finforme desc");
						...
						<table class="table table-responsive table-bordered align-middle">
							<thead class="thead-dark">
	                            <tr>
									...
									<th width="10%" class="text-center">Reproducción Asistida</th>
									<th width="10%" class="text-center">Procedimiento de Sala</th>
                            ...
                            <tbody>
	                            <?php while ($legal = $rLegal->fetch(PDO::FETCH_ASSOC)) { 
									...
									$ruta = "enfermeria/" . $dni . "/" . $legal['documento'];
									if (file_exists($ruta)) { 
								...
								?>
	                                ...
	                                <tr>
										...
										<td class="text-center"><?php print( mb_strtoupper($legal['medico']) ); ?></td>
	                                    <td class="text-center"><?php print( mb_strtoupper($legal['nombre_reproasistida']) ); ?></td>
										<td class="text-center"><?php print( mb_strtoupper($legal['nombre_procesala']) ); ?></td>
										<td><?php print( mb_strtoupper($legal['obs']) ); ?></td>
	                                    <?php echo $enlace; ?>
										<td class="text-center">
											<?php
											if ($user['role'] == 1 || $user['role'] == 12 || $user['role'] == 13 || $user['role'] == 15) {
												print("<img src='_libraries/open-iconic/svg/trash.svg' height='18' width='18' alt='icon name' class='btn_eliminar_informe' data-origen='enfermeria' data-informe='".$legal["id"]."'>");
											} ?>
										</td>
	                                </tr>
	                            <?php } ?>
