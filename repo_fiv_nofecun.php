...
<?php
    if ($_SESSION['role'] == "9") {
        ...
        $rPaci = $db->prepare("
            ...
            , lab_aspira.tip, hc_reprod.p_cic, hc_reprod.p_fiv, hc_reprod.p_icsi, hc_reprod.p_od, hc_reprod.p_don, hc_reprod.p_cri, hc_reprod.p_iiu
            , lab_aspira.fec
            having count( case when lab_aspira_dias.d1est = 'MII' and lab_aspira_dias.d1f_cic = 'N' and lab_aspira_dias.d1c_pol in ('0', '1', '2') and lab_aspira_dias.d1pron in ('0', '1', '2') then true end ) >= 1
        ...
        while ($paci = $rPaci->fetch(PDO::FETCH_ASSOC)) {
            ...
            print("
                <td>".$paci["med"]."</td>
                <td>".$paci["fec"]."</td>
                <td><a href='le_aspi1.php?id=".$paci['pro']."' target='_blank'>".$paci["nofecundado"]."</a></td></tr>");
        }
        print("
            </tbody>
        </table>");
?>
        <div class="ui-content" role="main" id="imprime">
            <div><p>Fecha de Inicio de la primera Reproducción: <?php if(isset($fecha_inicio))print($fecha_inicio); ?></p></div>
            <div class="scroll_h">
                <table style="border: 1px solid;" cellpadding="5">
                    <tbody>
                    <?php
                        print '<tr><td>Óvulos No Fecundados</td><td>'.$nofecundado.'</td></tr>';
                    ?>
                        <tr>
                            <td colspan="2" bgcolor="#ffe4c4"><?php print("Total:".($nofecundado) ); ?></td>
                        </tr>
                    </tbody>
                </table><br/><br/>
            </div>
