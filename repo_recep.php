...
<head>
    ...
    function PrintElem(elem) {
        ...
        mywindow.document.write("</head><body><p style='align: center'>Reporte Donación de Óvulos</p>");
...
<body>
    ...
    if ($_SESSION['role'] == "9") {
        ...
        $rPaci = $db->prepare("
            ...
            , lab_aspira.fec
            from hc_reprod
            inner join lab_aspira on lab_aspira.rep = hc_reprod.id and lab_aspira.estado is true and lab_aspira.f_fin is not null and lab_aspira.tip = 'R'
            left join hc_paciente on hc_paciente.dni = hc_reprod.dni
            left join hc_pareja on hc_pareja.p_dni = hc_reprod.p_dni
            where hc_reprod.estado = true and hc_reprod.cancela=0 and not (hc_reprod.des_don is not null and hc_reprod.des_dia >= 1)
...
?>
        <div class="ui-content" role="main" id="imprime">
            <div>
                <p>Fecha de Inicio de la primera Reproducción: <?php print($fecha_inicio); ?></p>
            </div>
            <div class="scroll_h">
                <table style="border: 1px solid;" cellpadding="5">
                    <tbody>
                        <?php
                        print('
                            <tr><td>Donación de Óvulos (OD Fresco)</td><td>'.$od.'</td></tr>
                            <tr><td>Donación de Óvulos (DESCONGELACIÓN OVULOS DONADOS)</td><td>'.$don.'</td></tr>');
                    ?>
                        <tr>
                            <td colspan="2" bgcolor="#ffe4c4"><?php print("Total de Donaciones:".($item-1) ); ?></td>
                        </tr>
                    </tbody>
                </table><br /><br />
            </div>
            <div style="float:right">
                <p><b>Fecha y Hora de Reporte:</b> <?php
