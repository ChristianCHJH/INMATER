...
<div data-role="page" class="ui-responsive-panel" id="r_tanque" data-dialog="true">
    ...
    <?php
        if (isset($_POST['conf']) and !empty($_POST['conf']))
        {
            ...
            contraccionEliminar($_POST['conf'], mb_strtolower($login));
        }

        if (isset($_POST['nom']) and !empty($_POST['nom']))
        {
            require("_database/db_mantenimiento.php");
            contraccionInsertar($_POST['codigo'], mb_strtoupper($_POST['nombre_corto']), mb_strtoupper($_POST['nom']), mb_strtolower($login));
        }
	?>
    <div data-role="header">
        ...
        <h3>MANTENIMIENTO DE CONTRACCIÓN</h3>
    </div>
    <div class="ui-content" role="main">
            ...
            <div class="ui-bar ui-bar-a">
                <table style="margin: 0 auto;" width="100%">
                    <tr>
                        ...
                        <td width="20%">Nombre Informe
                            <input type="text" name="nombre_corto" id="nombre_corto" data-mini="true" required/>
                        </td>
                        <td width="10%">
        ...
        <form action="" method="post" data-ajax="false" name="form2">
            ...
            <table width="100%" class="table-stripe" style="font-size: small" data-filter="true" data-input="#filtro">
                ...
                <tbody>
                <?php
                    $consulta = $db->prepare("select id, codigo, nombre, nombre_corto from lab_contraccion where estado = 1");
                    $consulta->execute();
                    while ($item = $consulta->fetch(PDO::FETCH_ASSOC))
                    {
                        ...
                        <tr>
                            ...
                            <td>".mb_strtoupper($item["nombre_corto"])."</td>
                            <td><a href='javascript:anular(".$item["id"].");'>Eliminar</a></td>
                        </tr>");
                    }
                ?>
                </tbody>
