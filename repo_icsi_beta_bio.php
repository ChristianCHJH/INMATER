...
<?php
    if ($_SESSION['role'] == "9") {
        ...
        $rPaci = $db->prepare("
            ...
            , count( case when lab_aspira_t.beta = 3 then true end ) betabio
            from hc_reprod
            ...
            , lab_aspira.tip, hc_reprod.p_cic, hc_reprod.p_fiv, hc_reprod.p_icsi, hc_reprod.p_od, hc_reprod.p_don, hc_reprod.p_cri, hc_reprod.p_iiu
            , lab_aspira.fec
            having count( case when lab_aspira_t.beta = 3 then true end ) > 0
        ...
        <table data-role='table' class='table-stripe ui-responsive mayuscula'>
            <thead>
                <tr>
                    ...
                    <th>Médico</th>
                    <th>Fecha</th>
                    <th>Beta Bioquimico</th>
                </tr>
            </thead>
            <tbody>
        ");
        while ($paci = $rPaci->fetch(PDO::FETCH_ASSOC)) {
            $betabio+=$paci['betabio'];
            ...
            print("
                <td>".$paci["med"]."</td>
                <td>".$paci["fec"]."</td>
                <td><a href='le_aspi1.php?id=".$paci['pro']."' target='_blank'>".$paci["betabio"]."</a></td></tr>");
        }
        print("
            </tbody>
        </table>");
        ...
        <div class="ui-content" role="main" id="imprime">
            ...
            <div class="scroll_h">
                <table style="border: 1px solid;" cellpadding="5">
                    <tbody>
                    <?php
                        print '<tr><td>Betas ICSI Bioquimico</td><td>'.$betabio.'</td></tr>';
                    ?>
                        <tr>
                            <td colspan="2" bgcolor="#ffe4c4"><?php print("Total:".($betabio) ); ?></td>
                        </tr>
                    </tbody>
                </table><br/><br/>
            </div>
