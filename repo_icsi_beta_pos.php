...
<body>
    ...
    if ($_SESSION['role'] == "9") {
        ...
        $rPaci = $db->prepare("
            ...
            , count( case when lab_aspira_t.beta = 1 then true end ) betapos
            from hc_reprod
        ...
        while ($paci = $rPaci->fetch(PDO::FETCH_ASSOC)) {
            $betapos+=$paci['betapos'];
            if ($fecha_inicio_band && isset($paci["fec"])) {
                $fecha_inicio = $paci["fec"];
                $fecha_inicio_band = false;
            }
            if ($paci['p_fiv'] == 1 && $paci['p_icsi'] == 1) {
                $ambos++;
            ...
            //procedimiento-ini
            print("<td>");
            if ($paci['p_cic'] >= 1) {print("Ciclo Natural<br>");}
            if ($paci['p_fiv'] >= 1) {print("FIV<br>");}
            if ($paci['p_icsi'] >= 1) {print($_ENV["VAR_ICSI"] . "<br>");}
            if ($paci['p_od'] <> '') {print("OD Fresco<br>");}
            if ($paci['p_cri'] >= 1) {print("Crio Ovulos<br>");}
            if ($paci['p_iiu'] >= 1) {print("IIU<br>");}
            if ($paci['p_don'] == 1) {print("Donación Fresco<br>");}
            if ($paci['des_don'] == null && $paci['des_dia'] >= 1) {print("TED<br>");}
            if ($paci['des_don'] == null && $paci['des_dia'] === 0) {print("<small>Descongelación Ovulos Propios</small><br>");}
            if ($paci['des_don'] <> null && $paci['des_dia'] >= 1) {print("EMBRIODONACIÓN<br>");}
            if ($paci['des_don'] <> null && $paci['des_dia'] === 0) {print("<small>Descongelación Ovulos Donados</small><br>");}
            print("</td>");
            //procedimiento-fin
            print("
                <td>".$paci["med"]."</td>
                <td>".$paci["fec"]."</td>
                <td><a href='le_aspi1.php?id=".$paci['pro']."' target='_blank'>".$paci["betapos"]."</a></td></tr>");
        }
        print("
            </tbody>
        </table>"); ?>
        <div class="ui-content" role="main" id="imprime">
            ...
            <div class="scroll_h">
                <table style="border: 1px solid;" cellpadding="5">
                    <tbody>
                    <?php
                        print '<tr><td>Betas ICSI Positivas</td><td>'.$betapos.'</td></tr>';
                    ?>
                        <tr>
                            <td colspan="2" bgcolor="#ffe4c4"><?php print("Total:".($betapos) ); ?></td>
                        </tr>
                    </tbody>
                </table><br/><br/>
            </div>
